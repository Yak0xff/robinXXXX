...
import { FEATURED_LOGS } from '~/constant'
import type { LogsFrontMatter } from '~/types/mdx'
import { formatDate } from '~/utils/date'
import Twemoji from '../Twemoji'

export function FeaturedLogs({ logs }: { logs: LogsFrontMatter[] }) {
  let { t, i18n } = useTranslation()
  let lang = i18n.language

  return (
    <>
      {!logs.length && 'No posts found.'}
      {logs.slice(0, FEATURED_LOGS).map((frontMatter) => {
        let { tags, date, summary, title, slug, cover } = frontMatter
